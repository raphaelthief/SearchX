...
class DSStore:
    ...
    def next_byte(self):
        data = self.content[self.cursor] # Modified original code here content --> self.content
        self.cursor += 1
        return data

    def next_bytes(self, n):
        data = self.content[self.cursor:self.cursor + n] # Modified original code here content --> self.content
        self.cursor += n
        return data
...
##########################################################################################
def extract_ds_store_metadata(file_path):
    print(f"\n{Fore.YELLOW}--- .DS_Store Metadata ---{Fore.GREEN}")
    try:
        ...
        for record in ds_store.read():
            print(f"{Fore.GREEN}[+] {Fore.YELLOW}Record Name : {Fore.GREEN}{record.name}")
            for description in record.human_readable():
                print(f"\t{description}")

    except Exception as e:
        print(f"{Fore.RED}Error reading .DS_Store metadata : {e}")
